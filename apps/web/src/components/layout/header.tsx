...
const navLinks: { href: string; label: string; external?: boolean }[] = [
  ...
  { href: "https://compendium.kalamos.care", label: "Compendium", external: true },
            ...
            {navLinks.map((link) =>
              link.external ? (
                <a
                  key={link.href}
                  href={link.href}
                  className="px-3 py-2 rounded-md text-sm font-medium transition-colors text-warm-600 hover:text-warm-900 hover:bg-warm-50"
                >
                  {link.label}
                </a>
              ) : (
